...
import type { Simulation, SimulationStatus, StoryCard } from '@/lib/types'
...
export function SimulationViewerClient({ simulationId }: SimulationViewerClientProps) {
  ...
  useEffect(() => {
    ...
    // After all phases, load the simulation
    setTimeout(() => {
      loadSimulation()
    }, totalDelay)
  }, [])
  ...
  const handleBranchSelect = async (branchId: string) => {
    ...
    // Update chaos based on branch choice
    if (simulation) {
    ...
    // Show story cards after a brief delay
    setTimeout(() => {
              ...
              <div className="flex-1">
                ...
                {incidentData && (
                  <p className="text-muted-foreground">
                    Original event: {incidentData.incident.title}
                  </p>
                )}
              </div>
